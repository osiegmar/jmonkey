...
class ChunkNLTest {
    ...
    @ParameterizedTest
    @LecChunkSource(DataChunkType.NL)
    void readNL(final LecFile lecFile, final TreeIndex<LecChunk> nlNode) {
        final ChunkNL chunkNL = ChunkNLDecoder.decode(lecFile.readChunk(nlNode.chunk()));
        assertThat(chunkNL.soundIds()).hasSizeBetween(0, 19);
    }
