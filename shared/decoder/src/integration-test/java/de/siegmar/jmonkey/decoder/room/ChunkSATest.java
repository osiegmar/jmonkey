...
class ChunkSATest {
    ...
    @ParameterizedTest
    @LecChunkSource(DataChunkType.SA)
    void readSA(final LecFile lecFile, final TreeIndex<LecChunk> saNode) {
        final ChunkSA chunkSA = ChunkSADecoder.decode(lecFile.readChunk(saNode.chunk()));
        assertThat(chunkSA.scaleSlots()).hasSizeBetween(0, 4);
    }
