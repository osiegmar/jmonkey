...
class ChunkCCTest {
    ...
    @ParameterizedTest
    @LecChunkSource(DataChunkType.CC)
    void readCC(final LecFile lecFile, final TreeIndex<LecChunk> ccNode) {
        final ChunkCC chunkCC = ChunkCCDecoder.decode(lecFile.readChunk(ccNode.chunk()));
        assertThat(chunkCC.colorCycles()).hasSizeBetween(0, 2);
    }
