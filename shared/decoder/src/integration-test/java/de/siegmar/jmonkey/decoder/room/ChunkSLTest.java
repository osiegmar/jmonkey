...
class ChunkSLTest {
    ...
    @ParameterizedTest
    @LecChunkSource(DataChunkType.SL)
    void readSL(final LecFile lecFile, final TreeIndex<LecChunk> slNode) {
        final ChunkSL chunkSL = ChunkSLDecoder.decode(lecFile.readChunk(slNode.chunk()));
        assertThat(chunkSL.unknown()).isEqualTo(0);
    }
