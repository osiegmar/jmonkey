...
class ChunkAMTest {
    ...
    @ParameterizedTest
    @LecChunkSource(DataChunkType.AM)
    void readAM(final LecFile lecFile, final TreeIndex<LecChunk> amNode) {
        final ChunkAM chunkAM = ChunkAMDecoder.decode(lecFile.readChunk(amNode.chunk()));
        assertThat(chunkAM.songNo()).isBetween(1, 51);
        assertThat(chunkAM.type()).isBetween(5, 200);
    }
