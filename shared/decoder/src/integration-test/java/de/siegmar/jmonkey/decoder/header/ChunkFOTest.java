...
class ChunkFOTest {
    ...
    @ParameterizedTest
    @LecChunkSource(DataChunkType.FO)
    void readFO(final LecFile lecFile, final TreeIndex<LecChunk> foNode) {
        final ChunkFO chunkFO = ChunkFODecoder.decode(lecFile.readChunk(foNode.chunk()));
        assertThat(chunkFO.items()).hasSizeGreaterThan(0);
    }
