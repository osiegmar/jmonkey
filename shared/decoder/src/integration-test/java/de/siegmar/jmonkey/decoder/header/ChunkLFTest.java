...
class ChunkLFTest {
    ...
    @ParameterizedTest
    @LecChunkSource(DataChunkType.LF)
    void readLF(final LecFile lecFile, final TreeIndex<LecChunk> lfNode) {
        final ChunkLF chunkFO = ChunkLFDecoder.decode(lecFile.readChunk(lfNode.chunk()));
        assertThat(chunkFO.roomId()).isBetween(1, 98);
    }
