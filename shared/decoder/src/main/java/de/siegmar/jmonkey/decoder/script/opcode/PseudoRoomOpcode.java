...
public class PseudoRoomOpcode extends AbstractOpcode {

    private static final Set<Integer> OPCODES = Set.of(0xCC);

    public PseudoRoomOpcode() {
        super(OPCODES);
    }

    @Override
    public void doDecode(final int opcode, final EnhancedByteBuffer bb, final OpcodeDelegate opcodeDelegate) {
        final int room = bb.readU8();

        final List<Integer> aliases = new ArrayList<>();
        int i;
        while ((i = bb.readU8()) != 0) {
            Assert.assertThat(i > 0x80);
            aliases.add(i & 0x7F);
        }

        opcodeDelegate.pseudoRoom(room, aliases);
    }
