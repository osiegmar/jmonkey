...
/**
 * Decoder for {@code actorFromPos} opcode (original: {@code find-actor}).
 */
public class ActorFromPosOpcode extends AbstractOpcode {

    private static final Set<Integer> OPCODES = Set.of(0x15, 0x55, 0x95, 0xD5);

    public ActorFromPosOpcode() {
        super(OPCODES);
    }
    ...
    public void doDecode(final int opcode, final EnhancedByteBuffer bb, final OpcodeDelegate opcodeDelegate) {
        ...
        final OpParameter x = resolveParameter16(bb, opcode, 0);
        final OpParameter y = resolveParameter16(bb, opcode, 1);
        opcodeDelegate.actorFromPos(result, x, y);
    }
