...
public class StopObjectCodeOpcode extends AbstractOpcode {

    private static final Set<Integer> OPCODES = Set.of(0x00, 0xA0);

    public StopObjectCodeOpcode() {
        super(OPCODES);
    }
    ...
    public void doDecode(final int opcode, final EnhancedByteBuffer bb, final OpcodeDelegate opcodeDelegate) {
        if (opcode == 0x00) {
            opcodeDelegate.stopObjectCode();
        } else {
            opcodeDelegate.stopLocalGlobalScript();
        }
    }
